...
plots[["scatter_plotLCvsFN"]] <- ggplot(topT, aes(x = LiveCells, y = FixedNuclei)) +
  ...
  scale_color_manual(
    values = c(
      "#2F3C23",    # labels >2 LiveCells
      "#5F7048",       # points >1 LiveCells
     # "#7286AA",    # labels for genes <1
      "gray80",    # points < 1
      "#2F3C23",       # labels >1 LiveCells
      "#d9741a", # points >1 FixedNuclei
      "#a3a3a3",    # points for genes <1
      "#2F3C23",       # points for genes >1 LiveCells
      "#2F3C23"   # points for genes >2 LiveCells
    )
  ) +
  theme_mk +
  remove_grid +
...
plots[["1st_line"]] <- plots[["scatter_plotLCvsFN"]] + plots[["scatter_plotLNvsFN"]]+
   ...
   plot_annotation(
    tag_levels = list(c('C', ' '),  # Use 'a' for automatic labels ('a', 'b', 'c', 'd')
                      theme = theme(
